...
if(isset($_GET["mode"])&&$_GET["mode"]=="insert"){
    ...
    $sql="INSERT INTO `download_board` VALUES (null,'$q_userid','$username','$usernick','$q_subject','$q_content','$regist_day',0,'$upfile_name','$copied_file_name','$file_type');";
    ...
    $sql="SELECT num from `download_board` where id ='$userid' order by num desc limit 1;";
    $result = mysqli_query($conn,$sql);
    if (!$result) {
    ...
    $sql="SELECT `file_copied_0` from `download_board` where num ='$q_num';";
    $result = mysqli_query($conn,$sql);
    if (!$result) {
      alert_back('Error: 6' . mysqli_error($conn));
      // die('Error: ' . mysqli_error($conn));
    ...
    $sql ="DELETE FROM `download_board` WHERE num=$q_num";
    $result = mysqli_query($conn,$sql);
    if (!$result) {
      die('Error: ' . mysqli_error($conn));
    }
  ...
  if(isset($_POST['del_file']) && $_POST['del_file'] =='1'){
    ...
    //삭제된 파일 정보를 download_board에서 해당되는 필드에 수정을 해야된다.
    $sql="UPDATE `download_board` SET `file_name_0`='', `file_copied_0` ='' , `file_type_0` ='' WHERE `num`=$q_num;";
    $result = mysqli_query($conn,$sql);
    if (!$result) {
      die('Error: ' . mysqli_error($conn));
    }
  ...
  if(!empty($_FILES['upfile']['name'])){
    ...
    $sql="UPDATE `download_board` SET `subject`='$q_subject',`content`='$q_content',`regist_day`='$regist_day',`file_name_0`= '$upfile_name', `file_copied_0` ='$copied_file_name',`file_type_0`='$file_type' WHERE `num`=$q_num;";
    $result = mysqli_query($conn,$sql);
    if (!$result) {
      die('Error: ' . mysqli_error($conn));
    }
